...
function Car(make, model, year) {
  ...
  */
  ...
  Movie.prototype.changeRating = function (num) {
    const avrRating = this.rating + num / 2;
    return this.rating = avrRating;
  }
...
// These objects are the posts that the user will have saved to their account.
  ...
  // Once the User constructor function is created,
// write a prototype method for the User function. Name this method addSavedPost.
...
// Make sure to name the properties the same as described previously (id, title, rating).
  ...
  // Write a prototype method for the User constructor function named removeSavedPost that
// will take in one number parameter representing the post id.
// Use this id to find and remove the matching object in the savedPosts array.
  
  User.prototype.removeSavedPost = function (num) {
    const index = this.savedPosts.findIndex(obj => obj === num);
...
// savedPosts = [{id, title, rating}, {id, title, rating}]
